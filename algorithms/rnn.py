...
def rnn_anomaly_detection(algorithm_settings_id, fetched_data):
    ...
    # RNN model yapısını oluşturma
    model = Sequential([
        SimpleRNN(units=50, return_sequences=True, input_shape=(scaled_features.shape[1], 1)),
        SimpleRNN(units=50),
        Dense(units=1)
    ])
